...
namespace Dango.Quest
{
    public class ArrowUIScript : MonoBehaviour
    {
        ...
        {
            SetPos();
            var dir = targets[num].transform.position - playerpos.position;
        ...
        public void SetPos()
        {
            ...
            else if(QuestManager.Instance.GetQuest(1) is QuestDestination questDest2){
                for (int i = 0; i < floorDatas.Length; i++)
                {
                    if (questDest2.CurrentFloor == questFloorNum[i])
                    {
                        num = i;
                        LookPos = targets[i].transform.position;
                    }
